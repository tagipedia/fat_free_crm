...
module AccountsHelper
  ...
  def account_summary(account)
    [number_to_currency(account.opportunities.pipeline.map(&:weighted_amount).sum, precision: 0, unit: (defined?(Spree) ? Monetize.parse(Spree::Config[:currency]).symbol : FatFreeCrm::Setting.currency)),
  ...
  def account_select(options = {})
    ...
    accounts = ([@account.new_record? ? nil : @account] + Account.my(current_fat_free_crm_user).order(:name).limit(25)).compact.uniq
  ...
  def display_value(value)
    ...
    number_to_currency(value, precision: 0, unit: (defined?(Spree) ? Monetize.parse(Spree::Config[:currency]).symbol : FatFreeCrm::Setting.currency))
  end

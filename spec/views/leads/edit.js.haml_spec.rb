...
describe "/leads/edit" do
  ...
  before do
    login
    assign(:lead, @lead = build_stubbed(:lead, status: "new", user: current_fat_free_crm_user))
  ...
  it "edit: should hide previously open [Edit Lead] and replace it with lead partial" do
    params[:cancel] = nil
    assign(:previous, previous = build_stubbed(:lead, user: current_fat_free_crm_user))
  ...
  it "edit from lead landing page: should not attempt to hide [Convert Lead] if the lead is already converted" do
    params[:cancel] = "false"
    assign(:lead, build_stubbed(:lead, status: "converted", user: current_fat_free_crm_user))

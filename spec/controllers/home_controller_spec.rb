...
describe HomeController do
  ...
  describe "responding to GET /" do
    ...
    it "should get a list of activities" do
      activity = create(:version, item: create(:account, user: current_fat_free_crm_user))
    ...
    it "should not include views in the list of activities" do
      create(:version, item: create(:account, user: current_fat_free_crm_user), event: "view")
    ...
    it "should get a list of my tasks ordered by due_at" do
      task_1 = create(:task, name: "Your first task", bucket: "due_asap", assigned_to: current_fat_free_crm_user.id)
      task_2 = create(:task, name: "Another task for you", bucket: "specific_time", calendar: 5.days.from_now.to_s, assigned_to: current_fat_free_crm_user.id)
      task_3 = create(:task, name: "Third Task", bucket: "due_next_week", assigned_to: current_fat_free_crm_user.id)
      task_4 = create(:task, name: "i've assigned it to myself", user: current_fat_free_crm_user, calendar: 20.days.from_now.to_s, assigned_to: nil, bucket: "specific_time")

      create(:task, name: "Someone else's Task", user_id: current_fat_free_crm_user.id, bucket: "due_asap", assigned_to: create(:user).id)
    ...
    it "should not display completed tasks" do
      my_task = create(:task, user_id: current_fat_free_crm_user.id, name: "Your first task", bucket: "due_asap", assigned_to: current_fat_free_crm_user.id)
      create(:task, user_id: current_fat_free_crm_user.id, name: "Completed task", bucket: "due_asap", completed_at: 1.days.ago, completed_by: current_fat_free_crm_user.id, assigned_to: current_fat_free_crm_user.id)
    ...
    it "should get a list of my opportunities ordered by closes_on" do
      opportunity_1 = create(:opportunity, name: "Your first opportunity", closes_on: 15.days.from_now, assigned_to: current_fat_free_crm_user.id, stage: 'proposal')
      opportunity_2 = create(:opportunity, name: "Another opportunity for you", closes_on: 10.days.from_now, assigned_to: current_fat_free_crm_user.id, stage: 'proposal')
      opportunity_3 = create(:opportunity, name: "Third Opportunity", closes_on: 5.days.from_now, assigned_to: current_fat_free_crm_user.id, stage: 'proposal')
      opportunity_4 = create(:opportunity, name: "Fourth Opportunity", closes_on: 50.days.from_now, assigned_to: nil, user_id: current_fat_free_crm_user.id, stage: 'proposal')
    ...
    it "should get a list of my accounts ordered by name" do
      account_1 = create(:account, name: "Anderson", assigned_to: current_fat_free_crm_user.id)
      account_2 = create(:account, name: "Wilson", assigned_to: current_fat_free_crm_user.id)
      account_3 = create(:account, name: "Triple", assigned_to: current_fat_free_crm_user.id)
      account_4 = create(:account, name: "Double", assigned_to: nil, user_id: current_fat_free_crm_user.id)
  ...
  describe "responding to GET options" do
    ...
    it "should assign instance variables for user preferences" do
      @asset = create(:preference, user: current_fat_free_crm_user, name: "activity_asset", value: Base64.encode64("tasks".to_json))
      @user = create(:preference, user: current_fat_free_crm_user, name: "activity_user", value: Base64.encode64("Billy Bones".to_json))
      @duration = create(:preference, user: current_fat_free_crm_user, name: "activity_duration", value: Base64.encode64("two days".to_json))
  ...
  describe "responding to GET redraw" do
    ...
    it "should save user selected options" do
      ...
      expect(current_fat_free_crm_user.pref[:activity_asset]).to eq("tasks")
      expect(current_fat_free_crm_user.pref[:activity_user]).to eq("Billy Bones")
      expect(current_fat_free_crm_user.pref[:activity_duration]).to eq("two days")
    end
  ...
  describe "activity_user" do
    ...
    it "should find a user by email" do
      ...
      allow(controller).to receive(:current_fat_free_crm_user).and_return(@cur_user)

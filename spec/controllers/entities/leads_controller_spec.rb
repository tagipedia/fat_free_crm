...
describe LeadsController do
  ...
  describe "responding to GET index" do
    it "should expose all leads as @leads and render [index] template" do
      @leads = [create(:lead, user: current_fat_free_crm_user)]
    ...
    it "should filter out leads by status" do
      ...
      @leads = [
        create(:lead, status: "new", user: current_fat_free_crm_user),
        create(:lead, status: "contacted", user: current_fat_free_crm_user)
      ]

      # This one should be filtered out.
      create(:lead, status: "rejected", user: current_fat_free_crm_user)
    ...
    it "should perform lookup using query string" do
      @billy_bones   = create(:lead, user: current_fat_free_crm_user, first_name: "Billy",   last_name: "Bones")
      @captain_flint = create(:lead, user: current_fat_free_crm_user, first_name: "Captain", last_name: "Flint")
  ...
  describe "responding to GET show" do
    describe "with mime type of HTML" do
      before(:each) do
        @lead = create(:lead, id: 42, user: current_fat_free_crm_user)
        @comment = Comment.new
      end
    ...
    describe "lead got deleted or otherwise unavailable" do
      it "should redirect to lead index if the lead got deleted" do
        @lead = create(:lead, user: current_fat_free_crm_user)
        @lead.destroy
  ...
  describe "responding to GET new" do
    it "should expose a new lead as @lead and render [new] template" do
      @lead = build(:lead, user: current_fat_free_crm_user, campaign: nil)
  ...
  describe "responding to GET edit" do
    it "should expose the requested lead as @lead and render [edit] template" do
      @lead = create(:lead, id: 42, user: current_fat_free_crm_user, campaign: nil)
      @campaigns = [create(:campaign, user: current_fat_free_crm_user)]
  ...
  describe "responding to POST create" do
    describe "with valid params" do
      ...
      it "should copy selected campaign permissions unless asked otherwise" do
        ...
        @lead = build(:lead, campaign: @campaign, user: current_fat_free_crm_user, access: "Shared")
        allow(Lead).to receive(:new).and_return(@lead)
      ...
      it "should reload lead campaign if called from campaign landing page" do
        @campaign = create(:campaign)
        @lead = build(:lead, user: current_fat_free_crm_user, campaign: @campaign)
    ...
    describe "with invalid params" do
      it "should expose a newly created but unsaved lead as @lead and still render [create] template" do
        @lead = build(:lead, user: current_fat_free_crm_user, first_name: nil, campaign: nil)
        allow(Lead).to receive(:new).and_return(@lead)
        @campaigns = [create(:campaign, user: current_fat_free_crm_user)]
  ...
  describe "responding to PUT update" do
    describe "with valid params" do
      it "should update the requested lead, expose it as @lead, and render [update] template" do
        @lead = create(:lead, first_name: "Billy", user: current_fat_free_crm_user)
      ...
      it "should update lead status" do
        @lead = create(:lead, status: "new", user: current_fat_free_crm_user)
      ...
      it "should update lead source" do
        @lead = create(:lead, source: "campaign", user: current_fat_free_crm_user)
    ...
    describe "AJAX request" do
      ...
      describe "when called from Leads index page" do
        ...
        it "should get data for the sidebar" do
          @another_lead = create(:lead, user: current_fat_free_crm_user)
      ...
      describe "when called from campaign landing page" do
        before(:each) do
          @campaign = create(:campaign)
          @lead = create(:lead, user: current_fat_free_crm_user, campaign: @campaign)
          request.env["HTTP_REFERER"] = "http://localhost/campaigns/#{@campaign.id}"
        end
  ...
  describe "responding to GET convert" do
    it "should should collect necessary data and render [convert] template" do
      ...
      @lead = create(:lead, user: current_fat_free_crm_user, campaign: @campaign, source: "cold_call")
  ...
  describe "responding to PUT promote" do
    it "on success: should change lead's status to [converted] and render [promote] template" do
      ...
      @opportunity = build(:opportunity, user: current_fat_free_crm_user, campaign: @lead.campaign,
                                         account: @account)
      allow(Opportunity).to receive(:new).and_return(@opportunity)
      @contact = build(:contact, user: current_fat_free_crm_user, lead: @lead)
    ...
    it "should copy lead permissions to newly created account and opportunity when asked so" do
      ...
      @lead.save
      @account = build(:account, user: current_fat_free_crm_user, access: "Shared")
      ...
      @opportunity = build(:opportunity, user: current_fat_free_crm_user, access: "Shared")
      ...
      put :promote, params: { id: @lead.id, access: "Lead", account: { name: "Hello", access: "Lead", user_id: current_fat_free_crm_user.id }, opportunity: { name: "World", access: "Lead", user_id: current_fat_free_crm_user.id } }, xhr: true
    ...
    it "should assign lead's source to the newly created opportunity" do
      @lead = create(:lead, user: current_fat_free_crm_user, source: "cold_call")
    ...
    it "on failure: should not change lead's status and still render [promote] template" do
      @lead = create(:lead, id: 42, user: current_fat_free_crm_user, status: "new")
      @account = create(:account, id: 123, user: current_fat_free_crm_user)
  ...
  describe "responding to POST discard" do
    before(:each) do
      @attachment = create(:task, assigned_to: current_fat_free_crm_user)
      @model = create(:lead)
  ...
  describe "responding to POST auto_complete" do
    before(:each) do
      @auto_complete_matches = [create(:lead, first_name: "Hello", last_name: "World", user: current_fat_free_crm_user)]
    end
  ...
  describe "responding to GET redraw" do
    it "should save user selected lead preference" do
      ...
      expect(current_fat_free_crm_user.preference[:leads_per_page]).to eq(42)
      expect(current_fat_free_crm_user.preference[:leads_index_view]).to eq("long")
      expect(current_fat_free_crm_user.preference[:leads_sort_by]).to eq("leads.first_name ASC")
      expect(current_fat_free_crm_user.preference[:leads_naming]).to eq("after")
    end

    it "should set similar options for Contacts" do
      get :redraw, params: { sort_by: "first_name", naming: "after" }, xhr: true
      expect(current_fat_free_crm_user.pref[:contacts_sort_by]).to eq("contacts.first_name ASC")
      expect(current_fat_free_crm_user.pref[:contacts_naming]).to eq("after")
    end
    ...
    it "should select @leads and render [index] template" do
      @leads = [
        create(:lead, first_name: "Alice", user: current_fat_free_crm_user),
        create(:lead, first_name: "Bobby", user: current_fat_free_crm_user)
      ]
  ...
  describe "responding to POST filter" do
    it "should filter out leads as @leads and render :index action" do
      ...
      @leads = [create(:lead, user: current_fat_free_crm_user, status: "new")]

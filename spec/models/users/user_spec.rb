...
describe User do
  ...
  describe '#destroyable?' do
    describe "Destroying users with and without related assets" do
      before do
        @user = build(:user)
        @current_fat_free_crm_user = build(:user)
      end
      ...
      %w[account campaign lead contact opportunity].each do |asset|
        it "should not destroy the user if she owns #{asset}" do
          ...
          expect(@user.destroyable?(@current_fat_free_crm_user)).to eq(false)
        end
      ...
      it "should destroy the user" do
        expect(@user.destroyable?(@current_fat_free_crm_user)).to eq(true)
      end
    end
  end
